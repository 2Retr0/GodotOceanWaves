...
@export var tile_length := Vector2(50, 50) :
	set(value): tile_length = value; should_generate_spectrum = true; _tile_length = [value.x, value.y]
@export_range(0, 2) var time_scale := 1.0 :
	set(value): time_scale = value; _time_scale = [value]
...
@export_range(0, 1) var detail := 1.0 : # Note: This should be reduced as the number of cascades increases to avoid *too* much detail!
